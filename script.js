...
const likeNamesFor = (likes = {}) =>
  Object.values(likes)
    .map((value) => (typeof value === "string" ? value : "Użytkownik"))
...
authForms.forEach((form) => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (mode === "register") {
      ...
      const currentUser = { id, firstName, lastName, isAuthor: isAuthorCredentials(firstName, lastName, password) };
    ...
    if (!user && isAuthorCredentials(firstName, lastName, password)) {
      ...
      writeCurrentUser({ id, firstName, lastName, isAuthor: true });
...
postsFeed.addEventListener("click", async (event) => {
  ...
  if (heartButton.dataset.likeTarget === "comment") {
    ...
    if (comment.likes[user.id]) {
      ...
      comment.likes[user.id] = currentUserDisplay();
      playHeartSound();
...
window.addEventListener("scroll", scheduleDockContrast, { passive: true });
appScreen.addEventListener("scroll", scheduleDockContrast, { passive: true });
